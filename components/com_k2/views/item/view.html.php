...
class K2ViewItem extends JView {
	...
	function display($tpl = null) {
		...
		if (is_object($item->author->profile) && $item->author->profile->id > 0) {
		...
		if ($item->metadesc) {
			...
			$metaDescItem = preg_replace("#{(.*?)}(.*?){/(.*?)}#s", '', $item->introtext);
			$metaDescItem = K2HelperUtilities::wordLimit($metaDescItem, $params->get('metaDescLimit', 150));
